...
#include <stdlib.h>
#include <string.h>
#define MAX_NUM 80          //字符串最多个数
#define MAX_HASH_NUM 96     //哈希表最大容量为1.2倍的原字符串个数
#define MAX_STR_LEN 12      //字符串最长长度
...
void InitList(char Name[MAX_NUM][MAX_STR_LEN]);        //初始化姓名表函数
unsigned int ELFHash(char *str);        //面向字符串的Hash处理函数——ELF算法
void Create_Hash_Table(HNode *HashTable, char Name[MAX_NUM][MAX_STR_LEN]);       //哈希表构造函数，采用开散列方法来解决冲突
void GetASL(HNode *HashTable, char Name[MAX_NUM][MAX_STR_LEN]);     //求解哈希表的平均查找次数
...
int main()
{
    ...
    //Create_Hash_Table(HashTable,Name);
    //GetASL(HashTable, Name);
    /****调试代码****/
...
{
    FILE *fp;
    if((fp=fopen("name_short.txt","r"))==NULL){
...
void Create_Hash_Table(HNode *HashTable, char Name[MAX_NUM][MAX_STR_LEN])       //哈希表构造函数，采用开散列方法来解决冲突
{
    ...
    for (i = 0; i < MAX_NUM; i++)
    {
        HashTable[i].data = MAX_NUM;     //因为数据值只可能是0—MAX_NUM-1的Name数组下标值，故赋值MAX_NUM表示这里没有数据
        HashTable[i].next = NULL;       //元素初始化
    }
    //printf("%d",HashTable[3].data);
    ...
    for (i = 0; i < MAX_NUM; i++)
    {
        ...
        p = &HashTable[hash_num-1];       //令p指向对应关键码的哈希表存储位置
        count = 0;
        if((p->data) == MAX_NUM){   //如果p的data域为MAX_NUM，则说明此关键字位置未被占用，则将对应字符串存入哈希表结点即可
            HashTable[i].data = i;
        }
...
void GetASL(HNode *HashTable, char Name[MAX_NUM][MAX_STR_LEN])     //求解哈希表的平均查找次数
{
    ...
    for (int i = 0; i < MAX_NUM; i++)       //从第一个name开始顺次查找，以计算总查找次数
    {
        ...
        printf("第%d个数据串查找成功，查找了%d次，目前一共查找了%d次\n", i, single_count, total_count);
    }   //end for

    printf("Search_Done.\n");
    float ASL = (float)total_count / (float)MAX_NUM;
    printf("The Average Search Length(ASL) is %.2f\n", ASL);
}